...
declare module '@deepseek-ai/dsh-client-ui-slots' {
  interface SlotMap {
    ...
    /**
     * One page inside the MCP settings section. The section owner renders
     * localized entry labels as tabs and mounts each contribution inside its
     * corresponding tab panel. Options: `id` (tab key), `order` (tab order),
     * and `label` (registrant-localized tab text). Declared at runtime by the
     * feature that owns the MCP section; the type lives here so the section
     * and its tabs collaborate without depending on one another.
     */
    'settings.mcp.tab': { kind: 'list'; scope: 'root'; owner: SettingsMcpTabOwnerProps }
    /**
     * One page inside the Skills settings section. The section owner renders
     * localized entry labels as tabs and mounts each contribution inside its
     * corresponding tab panel. Options: `id` (tab key), `order` (tab order),
     * and `label` (registrant-localized tab text). Declared at runtime by the
     * feature that owns the Skills section; the type lives here so the section
     * and its tabs collaborate without depending on one another.
     */
    'settings.skills.tab': { kind: 'list'; scope: 'root'; owner: SettingsSkillsTabOwnerProps }
    /**
...
/** Owner share of an MCP tab (the section supplies nothing). */
export interface SettingsMcpTabOwnerProps {
  /** Marker field: tab owner props are intentionally empty. */
  children?: never
}

/** Owner share of a Skills tab (the section supplies nothing). */
export interface SettingsSkillsTabOwnerProps {
  /** Marker field: tab owner props are intentionally empty. */
  children?: never
}
