  ...
  const api: ApiProxy = {
    ...
    skills: {
      list: (request) => {
        ...
        return ok(request, {
          skills: [
            { name: 'fixture-demo', description: 'fixture 技能样本', whenToUse: '仅供 UI 目录渲染验收', modelInvocable: true, provider: 'filesystem', source: 'project-dsh' },
            { name: 'fixture-user-only', description: 'fixture 仅用户技能样本', modelInvocable: false, provider: 'filesystem', source: 'user-dsh' },
          ],
        })
      },
      write: (request) => {
    ...
    llm: {
      ...
      balance: request => ok(request, { balance: null }),
    },
...
export class FixtureApiClient extends AbstractApiClient {
  ...
  private dispatch(
    ...
    switch (method) {
      ...
      case 'llm.balance': return this.api.llm.balance(request)
    }
  }
