...
describe('ui-theme host', () => {
  it('registers, validates, and disposes the durable theme namespace with its fiber', async () => {
    ...
    await ctx.settings.update(ns, { preference: 'other-world' })
    expect(ctx.settings.get(ns)).toEqual({ preference: 'other-world' })
  ...
  it('renders the current durable preference and disposes the index transform', async () => {
    ...
    await ctx.settings.update(settingsNamespace(THEME_SETTINGS_NAMESPACE), { preference: 'other-world' })
    expect(transform?.('<body></body>')).toContain('const preference = "other-world"')
    await fiber.dispose()
