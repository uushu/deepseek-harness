...
/** Every localStorage key the layer re-reads on a cross-tab storage event. */
const WATCHED_KEYS = new Set([
  ...Object.values(NUMERIC_KEYS),
  BACKGROUND_KEY, WALLPAPER_KEY, MODE_KEY,
  WHALE_KEY, CRITTERS_KEY, MESH_KEY, SPOTLIGHT_KEY, PRESS_KEY,
])
  ...
  constructor(ctx: Context) {
    ...
    ctx.effect(() => {
      const onStorage = (event: StorageEvent): void => {
        ...
        const key = event.key
        if (key !== null && WATCHED_KEYS.has(key)) {
          this.reloadSettings()
        ...
        if (wallpaper.startsWith('idb:')) {
          ...
          if (this.videoBlobId === wallpaper && this.videoObjectUrl !== undefined) {
            ...
            void loadVideoHandle().then(async (handle) => {
              ...
              if (this.settings.wallpaper !== wallpaper) return // stale load
              try {
                // queryPermission is part of the File System Access API but
                // not in every TS DOM lib — feature-detect it defensively.
                ...
                if (probe.queryPermission !== undefined) {
                  const permission = await probe.queryPermission({ mode: 'read' })
