...
export const CLIENT_SLOT_API: readonly ClientSlotEntry[] = [
  ...
  },
  {
    ...
    declaredBy: 'an entry in \'conversation\' (client-ui-conversation), so it exists while that entry is mounted',
    occupants: [
      'client-ui-conversation StatsLineWithBalance id \'stats\'',
    ],
    replaceRisk: 'none',
  ...
  },
  {
    ...
    source: 'packages/client/ui-settings/src/client/contract/slots.ts:106',
  },
  {
    key: 'settings.header',
    kind: 'single',
  ...
  },
  {
    ...
    hookContext: '',
    slotInject: '',
    declaredBy: 'an entry in \'settings.section\' (client-ui-settings-mcp), so it exists while that entry is mounted',
    occupants: [
      'client-ui-settings-mcp McpConfigTab id \'config\'',
      'client-ui-settings-mcp McpInventoryTab id \'inventory\'',
    ],
    replaceRisk: 'none',
    example: 'return {\n  inject: [\'slots\'],\n  apply(ctx) {\n    ctx.slots.inject(\'settings.mcp.tab\', () => ctx.slots.register(\n      { name: \'settings.mcp.tab\', id: \'my-entry\', order: 100, label: \'My entry\' },\n      () => React.createElement(\'div\', null, \'hello\'),\n    ))\n  },\n}',
  ...
  },
  {
    ...
    source: 'packages/client/ui-settings/src/client/contract/slots.ts:91',
  },
  {
    ...
    keyDomain: 'open: any string the owner dispatches (no compile-time key set), already taken: settings.aqua',
    hookContext: '',
    slotInject: '',
    declaredBy: 'an entry in \'settings.plugins.tab\' (client-ui-settings-plugins), so it exists while that entry is mounted',
    occupants: [
      'client-ui-harness AquaPluginCard key \'settings.aqua\'',
  ...
  },
  {
    ...
    occupants: [
      ...
      'client-ui-harness ThemeSection id \'theme\'',
      'client-ui-settings-general GeneralSection id \'general\'',
      'client-ui-settings-general PersonalizationSection id \'personalization\'',
      ...
      'client-ui-workspace DeletedConversationsSection id \'deleted-conversations\'',
      'client-ui-workspace ArchivedConversationsSection id \'archived-conversations\'',
    ],
    replaceRisk: 'none',
  ...
  {
    key: 'settings.skills.tab',
    kind: 'list',
    scope: 'root',
    summary: 'One page inside the Skills settings section.',
    doc: 'One page inside the Skills settings section. The section owner renders\nlocalized entry labels as tabs and mounts each contribution inside its\ncorresponding tab panel. Options: `id` (tab key), `order` (tab order),\nand `label` (registrant-localized tab text). Declared at runtime by the\nfeature that owns the Skills section; the type lives here so the section\nand its tabs collaborate without depending on one another.',
    registerOptions: [
      {
        name: 'id',
        requirement: 'required',
    ...
    hookContext: '',
    slotInject: '',
    declaredBy: 'an entry in \'settings.section\' (client-ui-settings-skills), so it exists while that entry is mounted',
    occupants: [
      'client-ui-settings-skills SkillsConfigTab id \'config\'',
      'client-ui-settings-skills SkillsListTab id \'list\'',
    ],
    replaceRisk: 'none',
    example: 'return {\n  inject: [\'slots\'],\n  apply(ctx) {\n    ctx.slots.inject(\'settings.skills.tab\', () => ctx.slots.register(\n      { name: \'settings.skills.tab\', id: \'my-entry\', order: 100, label: \'My entry\' },\n      () => React.createElement(\'div\', null, \'hello\'),\n    ))\n  },\n}',
