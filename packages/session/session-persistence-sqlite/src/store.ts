...
export class SqliteStore implements PersistenceBackend<number> {
  ...
  /**
   * Rewrite one materialized session's cwd and advance its revision.
   * @param id - persisted session to retarget.
   * @param cwd - new absolute working directory.
   * @returns resolution after the metadata mutation commits.
   */
  async retargetCwd(id: SessionId, cwd: string): Promise<void> {
    await this.open()
    validateSchemaForMutation(this.databaseConstructor, this.db, this.databasePath)
    const result = this.db.prepare(sql('update-session-cwd')).run(cwd, id)
  ...
  /**
   * Delete one materialized session and its cascading event rows.
   * @param id - persisted session to remove.
   * @returns resolution after the idempotent deletion.
   */
